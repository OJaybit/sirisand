...
export default function ReviewTestimonial() {
  ...
  const maxIndexRef = useRef(maxIndexLg);

  useEffect(() => {
    maxIndexRef.current = maxIndexLg;
  }, [maxIndexLg]);

  useEffect(() => {
    const interval = setInterval(() => {
      setIndex(prev => {
        return prev >= maxIndexRef.current ? 0 : prev + 1;
      });
    }, 6000);
      ...
      <div className="container mx-auto px-6 lg:px-20">
        ...
        <div className="hidden lg:block relative">
          <div className="grid grid-cols-4 gap-6">
            {testimonials.slice(index, index + 4).map((t) => (
              <div key={t.id}>{Card(t)}</div>
            ))}
          </div>

          {index > 0 && (
            <button onClick={prevLg} className="absolute -left-6 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow text-black">
              <ChevronLeft />
            </button>
          )}

          {index < maxIndexLg && (
            <button onClick={nextLg} className="absolute -right-6 top-1/2 -translate-y-1/2 bg-white p-2 rounded-full shadow text-black">
              <ChevronRight />
            </button>
          )}
        </div>
        ...
        <div className="lg:hidden overflow-hidden relative">
          <motion.div
            ...
            onDragEnd={(event, info) => {
              ...
              if (offset < -50) setIndex((prev) => Math.min(prev + 1, testimonials.length - 1));
              if (offset > 50) setIndex((prev) => Math.max(prev - 1, 0));
            }}
            animate={{ x: `-${index * 100}%` }}
          ...
          {/* MOVED TRACKER OUTSIDE THE SLIDER */}
          <div className="mt-6">
            <div className="relative h-1 w-full max-w-[120px] mx-auto bg-gray-200 rounded-full">
              <motion.div
                ...
                animate={{ x: (index / (testimonials.length - 1)) * 96 }}
